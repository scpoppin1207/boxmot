...
def mutual_nearest_neighbor_matching(cost_matrix, thresh=1.0):
    """
    基于互为最近邻的匹配算法
    :param cost_matrix: 距离矩阵，值越小表示越相似
    :param thresh: 距离阈值，超过此阈值的匹配将被忽略
    :return: 匹配对、未匹配的视角A索引、未匹配的视角B索引
    """
    if cost_matrix.size == 0:
        return (
            np.empty((0, 2), dtype=int),
    ...
    # 对每个视角A中的对象，找到视角B中距离最小的对象
    nn_a = np.argmin(cost_matrix, axis=1)
    # 对每个视角B中的对象，找到视角A中距离最小的对象
    nn_b = np.argmin(cost_matrix, axis=0)
    
    # 检查是否互为最近邻
    for i in range(n):
        j = nn_a[i]  # 视角A中第i个对象的最近邻是视角B中的第j个对象
        if nn_b[j] == i and cost_matrix[i, j] < thresh:  # 互为最近邻且距离小于阈值
            matches.append([i, j])
    
    if len(matches) > 0:
        matches = np.asarray(matches)
        # 找出未匹配的对象
        matched_a = set(matches[:, 0])
        matched_b = set(matches[:, 1])
        unmatched_a = tuple(i for i in range(n) if i not in matched_a)
        unmatched_b = tuple(j for j in range(m) if j not in matched_b)
    else:
        matches = np.empty((0, 2), dtype=int)
        unmatched_a = tuple(range(n))
        unmatched_b = tuple(range(m))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--match_method', type=str, default='mnn', choices=['mnn', 'lap'], 
                      help='匹配算法: mnn(互为最近邻)或lap(线性分配)')
    ...
    # 根据命令行参数选择匹配算法
    if args.match_method == 'mnn':
        print("使用互为最近邻匹配算法")
        matches, u_a, u_b = mutual_nearest_neighbor_matching(emb_dists, MAX_FEATURE_DIST)
    else:
        print("使用线性分配匹配算法")
        matches, u_a, u_b = linear_assignment(emb_dists, MAX_FEATURE_DIST)
